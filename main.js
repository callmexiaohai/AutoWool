...
ui.layout(
    <drawer id="drawer">
        <vertical>
            <appbar>
                <toolbar bg="#FF5c50e6" id="toolbar" title="我的私房钱v1.0.4" paddingTop="2dp" h="auto" >
                </toolbar>
                <tabs id="tabs" />
            </appbar>
...
ui.通用执行.click(function () {
    ...
    // }
    // var tipMessage = "本次共" + appArray.length + "个App参与任务，共循环"
    ...
    //             if(files.isFile('./modules/wool.js')){
    //                 wools.wool(appArray, foreachTimes, screenSileTimes, isShowConsole, timesInterval);
...
ui.执行日常任务.click(function () {
    ...
    var tipMessage = "本次共" + appArrayrc.length + "个App参与任务，共循环"
        + "确认执行吗？如果配置不正确请点击取消，前往配置页面进行参数修正！";
    confirm(tipMessage).then(value => {
        //当点击确定后会执行这里, value为true或false, 表示点击"确定"或"取消"
        if (value) {
...
ui.复制.on("item_bind", function (itemView, itemHolder) {
    itemView.复制邀请码.on("click", function () {
