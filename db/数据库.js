...
//打开或创建test.db数据库        
db  =  context.openOrCreateDatabase(files.cwd()+"/db/cedict.db",  Context.MODE_PRIVATE,  null);        



// //创建person表
db.execSQL("create table if not exists " +  "版本" + "(_id integer primary key autoincrement,name,age)");  




// //遍历出表名
// var cursor = db.rawQuery("select name from 版本 where type='table' AND name NOT LIKE 'android_metadata' order by name", null);
// log('遍历出表名:',cursor.getCount());
...
 //插入ContentValues中的数据        
db.insert("版本",  null,  cv); 


                
//  查询  c 是 Cursor类
var c = db.query("版本", null, "age<38", null, null, null, null, null);        
while  (c.moveToNext())  {              
